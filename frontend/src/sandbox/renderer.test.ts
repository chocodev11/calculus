...
describe('sandbox renderer', () => {
  it('renders each first-release scene as escaped svg', () => {
    ...
    expect(renderSceneSvg({ kind: 'logic', space: 'predicate_playground', expressionLabel: 'P(x): x > x^3', domainLabel: 'D', elements: [{ input: '1/3', value: 1 / 3, truthValue: true }], labels: [] })).toContain('Tập đúng trong miền khảo sát')
