        ...
        >
          ...
          {statsData && (
            <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${
              activeTab === 'achievements' ? 'bg-purple-100 text-purple-600' : 'bg-slate-200 text-slate-500'
            }`}>
...
function AchievementCard({ achievement, current, unit, gradientBar }) {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 transition-colors ${
      earned ? 'bg-white hover:bg-slate-50' : 'bg-slate-50/60'
    }`}>
      {/* Icon */}
      <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-2xl flex-shrink-0 ${
        earned ? 'bg-white shadow-sm border border-slate-100' : 'opacity-40 grayscale'
      }`}>
        {achievement.icon || '🏅'}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide flex-shrink-0 ${
            earned ? rarity.chip : 'bg-slate-100 text-slate-400'
          }`}>{rarity.text}</span>
        </div>
        <p className={`text-[11px] leading-snug mb-1.5 ${
          earned ? 'text-slate-500' : 'text-slate-400'
